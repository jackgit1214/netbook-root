...
import {queryHeight} from 'assets/jss/material-dashboard-react.js';
...
function TaskPage(props) {
    ...
    React.useEffect(() => {
        ...
        if (code == "99") {
            stopTaskMonitor(dataInfo);
        }
    }, [message]);

    const stopTaskMonitor = (task) => {
        let tmpEqual = (element) => element.taskId == task.taskId;
        let tmpIndex = taskMap.findIndex(tmpEqual);
        if (tmpIndex != -1) {
            let tmpObj = taskMap[tmpIndex];
            clearInterval(tmpObj.interval);
            taskMap.splice(tmpIndex, 1) //删除当前执行的任务
        }
    };
    React.useEffect(() => {
        retrieveData({
                ...
                <Grid container alignItems={'center'}>
                    {taskList.map((task, i) => {
                        const handleStartTask = () => {
                            ...
                            let tmp = taskMap;
                            tmp.push({taskId: task.taskId, interval: test})
                            setTaskMap(tmp);
                        };
                        const handleStopTask = () => {
                            stopTaskMonitor(task); //先停止监控任务
                            ...
                            <Grid item className={classes.gridItem} key={"parent" + i}>
                                <Card className={classes.cardWrapper}>
                                    <CardHeader className={classes.cardHeaderRoot}
                                                avatar={
                                                    <Avatar aria-label="recipe" style={{fontSize: 12, color: "red"}}
                                                ...
                                                action={
                                                    <IconButton aria-label="settings"
                                                                className={classes.cardActionButton} onClick={() => {
                                                        openEditTask(task)
                                                    }
                                                    }>
                                                        <EditIcon/>
                                    ...
                                    <CardContent className={classes.cardContentRoot}>
                                        <Grid container key={"child" + i}>
                                            ...
                                            <Grid item xs={7}>
                                                <Typography
                                                    className={classes.secondaryHeading}>{task.seedUrl}</Typography>
                                            </Grid>
                                            <Grid item xs={3}>
                                                <Typography align={"right"}
                                                            className={classes.heading}>匹配规则：</Typography>
                                            </Grid>
                                            <Grid item xs={7}>
                                                <Typography
                                                    className={classes.secondaryHeading}>{task.matchRules}</Typography>
                                            </Grid>
                                        </Grid>
                                    </CardContent>
                                    <CardActions disableSpacing className={classes.cardActionRoot}>

                                        <IconButton aria-label="taskHandle"
                                                    onClick={task.taskState == "1" ? handleStopTask : handleStartTask}
                                                    className={classes.cardActionButton}>
                                            <Tooltip title={task.taskState == "1" ? "终止任务" : "开始任务"}>
                                                {task.taskState == '1' ? <StopIcon fontSize="small"/> :
                                                    <PlayCircleOutlineIcon fontSize="small"/>}
                                            </Tooltip>
                                        </IconButton>
                                        <IconButton aria-label="taskHandle" onClick={handleErrorRecord}
                                                    className={classes.cardActionButton}>
                                            <Tooltip title="重新抓取错误页">
                                                <SpellcheckIcon fontSize="small" color="error"/>
                                            </Tooltip>
                                        </IconButton>
                                        <IconButton aria-label="taskHandle" onClick={handleTaskToBook}
                                                    className={classes.cardActionButton}>
                                            <Tooltip title="任务处理,生成书籍">
                                                <SaveAltIcon fontSize="small" color="primary"/>
                                            </Tooltip>
                                        </IconButton>
                                        <IconButton style={{marginLeft: 'auto'}} onClick={handleDeleteTask}
                                                    aria-label="delete task" className={classes.cardActionButton}>
                                            <Tooltip title="删除任务">
                                                <DeleteIcon/>
                                            </Tooltip>
                                        </IconButton>
                                        <IconButton style={{marginLeft: 'auto'}} onClick={handleWeiInfo}
                                                    aria-label="delete task" className={classes.cardActionButton}>
                                            <Tooltip title="测试">
                                                <DeleteIcon/>
                                            </Tooltip>
